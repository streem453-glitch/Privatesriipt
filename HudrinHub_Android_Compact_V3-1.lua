--[[
    ...
    - Aim Assist and ESP target real players in the current Roblox game.
...
do
    ...
    local section = Section(
        page,
        "Player Aim",
        "Aim Assist выбирает реальных игроков в пределах FOV.",
        86
    )
...
do
    ...
    Section(
        page,
        "Player ESP",
        "ESP показывает реальных игроков, включая тех, кто появляется позже.",
        76
    )
...
local function GetPlayerRole(player)
    local role = player:GetAttribute("Role")
    if typeof(role) == "string" and State.ESP[role] then
        return role
    end
    ...
    if character then
        ...
        local value = character:FindFirstChild("Role")
        if value and value:IsA("StringValue") and State.ESP[value.Value] then
            return value.Value
        end
    end
...
local function GetTargetPart(model)
    ...
    if State.AimPart == "Head" then
        preferred = {"Head", "UpperTorso", "HumanoidRootPart", "Torso", "LowerTorso"}
    else
        preferred = {"HumanoidRootPart", "UpperTorso", "Torso", "LowerTorso", "Head"}
    end
...
local function RefreshTargetCache()
    TargetCache = GetPlayerCharacters()
    LastTargetScan = os.clock()
end

local function FindBestPlayerTarget()
    local camera = workspace.CurrentCamera
    if not camera then
        return nil
    end

    if os.clock() - LastTargetScan > 0.10 then
        RefreshTargetCache()
    end
    ...
    for _, character in ipairs(TargetCache) do
        local player = GetPlayerFromModel(character)
        if IsValidPlayer(player) then
            local humanoid = character:FindFirstChildOfClass("Humanoid")
...
RunService.RenderStepped:Connect(function()
    ...
    local target = FindBestPlayerTarget()
    if target then
        local targetCFrame = CFrame.lookAt(camera.CFrame.Position, target.Position)
        local alpha = math.clamp(State.AimSmoothness, 0.05, 0.98)
        camera.CFrame = camera.CFrame:Lerp(targetCFrame, alpha)
    end
end)

Players.PlayerAdded:Connect(function()
    task.defer(RefreshTargetCache)
end)
...
local function CreateESP(model)
    ...
    local root = model:FindFirstChild("HumanoidRootPart") or GetTargetPart(model)
    if not root then
        return
    end
    ...
    if settings.Highlight then
        ...
        highlight.Name = "HudrinPlayerESPHighlight"
    ...
    if settings.Box or settings.Name or settings.Distance then
        local billboard = Instance.new("BillboardGui")
        billboard.Name = "HudrinPlayerESP"
...
local function UpdateESP()
    for model in pairs(ESPObjects) do
        local player = GetPlayerFromModel(model)
        if not model.Parent or not IsValidPlayer(player) then
            RemoveESP(model)
        end
    end
            ...
            if settings and settings.Enabled then
                ...
                if data then
                    if data.Highlight then
                        ...
                        data.Highlight.FillColor = settings.Color
                        data.Highlight.OutlineColor = settings.Color
                    end
                    ...
                    if data.Label then
                        ...
                        data.Label.TextColor3 = settings.Color
                    end
                end
